...
class TokenizeChargeRequest extends AbstractBusinessRequest
{
    ...
    public function setOrderData(WC_Order $order)
    {
        ...
        // if card on file, use token, otherwise use nonce
        if (! empty($order->payment->token)) {
            $cardField = 'cardToken';
            $fieldValue = $order->payment->token;
        } elseif (! empty($order->payment->nonce)) {
            $cardField = 'nonce';
            $fieldValue = $order->payment->nonce;
        } else {
            $cardField = 'cardToken';
            $fieldValue = ''; // should not happen
        }
