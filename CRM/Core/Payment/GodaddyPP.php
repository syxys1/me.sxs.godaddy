...
/*
 +----------------------------------------------------------------------------+
 | Godaddy In Person and Gateway Extension Merchant Payment Module for        |
 | CiviCRM version 5                                                          |
 +----------------------------------------------------------------------------+
 | Licensed to CiviCRM under the Academic Free License version 3.0            |
 |                                                                            |
 | Based on work from Eileen McNaughton - Nov March 2008                      |
 | Written by Sylvain Plante - October 2024                                   |
 +----------------------------------------------------------------------------+
 */
...
class CRM_Core_Payment_GodaddyPP extends CRM_Core_Payment {
  ...
  /**
   * This support variable is used to allow the capabilities supported by the processor to be set from unit tests
   * so that we don't need to create a lot of new processors to test combinations of features.
   * Initially these capabilities are mostly set to FALSE, however they can be altered by calling the setSupports function directly from outside the class.
   * @var bool[]
   */
  protected $supports = [
  ...
  public function setDoDirectPaymentResult($doDirectPaymentResult) {
    Civi::log()->debug('GodaddyPP.php::setDoDirectPaymentResult ' . $doDirectPaymentResult);
  ...
  public function doPayment(&$params, $component = 'contribute') {
    ...
    if (isset($params['is_recur']) && $params['is_recur'] == TRUE) {
      throw new CRM_Core_Exception(ts('Godaddy - recurring payments not implemented'));
    }
    ...
    //CRM_Godaddy_Utils::myPushInvoiceToGodaddy($this->_paymentProcessor, $requestFields);
  ...
  public function handlePaymentNotification() {
    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification');
        
    $rawData = file_get_contents("php://input");
    //Civi::log()->debug('GodaddyPP.php::handlePaymentNotification rawData' . '  ' . print_r($rawData, true));
    $rawData = json_decode($rawData, true);
    //Civi::log()->debug('GodaddyPP.php::handlePaymentNotification rawData after json_decode' . '  ' . print_r($rawData, true));
    $ipnClass = new CRM_Core_Payment_GodaddyIPN($rawData);
    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification ipnClass' . '  ' . print_r($ipnClass, true));

    
    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $rawData merchant_id' . '  ' . print_r($rawData["merchant_id"], true));
    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $rawData event_id' . '  ' . print_r($rawData["event_id"], true));
    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $rawData created_at' . '  ' . print_r($rawData["created_at"], true));
    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification switch $rawData type' . '  ' . print_r($rawData["type"], true));
    $subData = $rawData["data"];

    switch ($rawData["type"]) {
      case "payment.created":
      case "payment.updated":
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification payment.created');
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification payment.updated');
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $rawData data' . '  ' . print_r($rawData["data"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $subData type' . '  ' . print_r($subData["type"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $subData id' . '  ' . print_r($subData["id"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $subData object' . '  ' . print_r($subData["object"], true));
        $payData = $subData["object"]["payment"];
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData amount_money' . '  ' . print_r($payData["amount_money"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData application_details' . '  ' . print_r($payData["application_details"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData approved_money' . '  ' . print_r($payData["approved_money"], true));
        //Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData capabilities' . '  ' . print_r($payData["capabilities"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData card_details' . '  ' . print_r($payData["card_details"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData created_at' . '  ' . print_r($payData["created_at"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData customer_id' . '  ' . print_r($payData["customer_id"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData id' . '  ' . print_r($payData["id"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData location_id' . '  ' . print_r($payData["location_id"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData order_id' . '  ' . print_r($payData["order_id"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData receipt_number' . '  ' . print_r($payData["receipt_number"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData reference_id' . '  ' . print_r($payData["reference_id"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData source_type' . '  ' . print_r($payData["source_type"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData status' . '  ' . print_r($payData["status"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData total_money' . '  ' . print_r($payData["total_money"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData updated_at' . '  ' . print_r($payData["updated_at"], true));
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData version' . '  ' . print_r($payData["version"], true));
        
        switch ($payData["status"]) {
          case "APPROVED":
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData status APPROVED - wait for completed');
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData status' . '  ' . print_r($payData["status"], true));
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData version' . '  ' . print_r($payData["version"], true));
            break;
          case "COMPLETED":
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData status COMPLETED');
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData status' . '  ' . print_r($payData["status"], true));
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $payData version' . '  ' . print_r($payData["version"], true));
            
            $cardData = $payData["card_details"];
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $cardData' . '  ' . print_r($cardData, true));
            ...
            if ($this->_mode === 'test') {
              ...
              Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $params' . '  ' . print_r($params, true));
              ...
              $contributions = \Civi\Api4\Contribution::get(FALSE)
                ...
                ->execute();
              Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $contributions' . '  ' . print_r($contributions, true));
              
              foreach ($contributions as $contribution) {
                Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $contribution id' . '  ' . print_r($contribution["id"], true));
                if (strcasecmp($contribution["contact_id"], $payData["customer_id"]) <> 0) {
                  Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $contribution contact_id' . ' ' .
                ...
                if (strcasecmp($contribution["currency"], $payData["amount_money"]["currency"]) <> 0) {
                  Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $contribution currency' . '  ' . 
                ...
                Civi::log()->debug('GodaddyPP.php::handlePaymentNotification x' . '  ' . print_r($x, true));
                Civi::log()->debug('GodaddyPP.php::handlePaymentNotification payData' . '  ' . print_r((int)$payData["amount_money"]["amount"] / 100, true));
                Civi::log()->debug('GodaddyPP.php::handlePaymentNotification contribution' . '  ' . print_r((int)$contribution["balance_amount"], true));
                   
                switch (true) {
                  case $x == 0:
                    $contribution["contribution_status_id"] = 1;
                    $contribution["trxn_id"] = $payData['id'];
                    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $contribution trxn_id' . '  ' . print_r($contribution["trxn_id"], true));
                    break;
                  case $x < 0:
                    Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $contribution balance amount' . '  ' . 
                ...
                foreach ($results as $result) {
                  Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $contribution update result' . '  ' . print_r($result, true));
                    // do something
                }
                // do something
              }
              ...
              $result = $this->setStatusPaymentCompleted($result);
              Civi::log()->debug('GodaddyPP.php::handlePaymentNotification $result' . '  ' . print_r($result, true));
              return $result;
            }
            break;
          default:
            Civi::log()->debug('GodaddyPP.php::handlePaymentNotification other status' . ' ' . print_r($payData["status"], true));
        }
        break;
      case "order.created":
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification order.created');
        break;
      case "order.updated":
        Civi::log()->debug('GodaddyPP.php::handlePaymentNotification order.updated');
        break;
      default:
      Civi::log()->debug('GodaddyPP.php::handlePaymentNotification undefined response');
    }
    ...
    if ($ipnClass->onReceiveWebhook()) {
      ...
      Civi::log()->debug('GodaddyPP.php::handlePaymentNotification ipnClass ReceiveWebhook' . '  ' . print_r($ipnClass->onReceiveWebhook(), true));
    }
  }
